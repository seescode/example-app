...
import { BookActionTypes, SearchBooksComplete } from '../actions/book';
...
export class BookEffects {
  ...
  @Effect() loadCollection$ = this.actions$
    ...
    .startWith(new collection.LoadCollection())
    .switchMap(() => this.db.query('books').toArray())
    .map((books: Book[]) => new collection.LoadCollectionSuccess(books));
  ...
  @Effect() search$ = this.actions$
    ...
    .switchMap(query => {
      ...
      return this.googleBooks.searchBooks(query)
        ...
        .map(books => new SearchBooksComplete(books))
        .catch(() => Observable.of(new SearchBooksComplete([])))
    });
  ...
  @Effect() addBookToCollection$ = this.actions$
    ...
    .mergeMap(book => this.db.insert('books', [ book ])
      ...
      .catch(() => Observable.of(
        new collection.AddBookFail(book)
      ))
    );
  ...
  @Effect() removeBookFromCollection$ = this.actions$
    ...
    .mergeMap(book => this.db.executeWrite('books', 'delete', [ book.id ])
      ...
      .catch(() => Observable.of(
        new collection.RemoveBookFail(book)
      ))
    );
}
